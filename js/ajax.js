...
function creadTable() {
    ...
    for (var i = 0; i < datos.length; i++) {
        detalle += "<tr>" +
            "<td> <a id='" + datos[i].imdbID + "' onclick='ver_mas_detalles(this.id)' class='btn btn-more'>Ver mas</a></td>" + //onclick='ver_mas_detalles(this.innerText)'
            "<td>" + datos[i].Title + "</td>" +
            "<td>" + datos[i].Year + "</td>" +
...
function ver_mas_detalles(id_pelicula) {
    ...
    xmlhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            //alert("llegue"); 
            /* document.getElementById("informacion").innerHTML = this.responseText;  */
            cargarInfPelicula(this.response);
        }
    };
    /* xmlhttp.open("GET","../../controladores/user/insertarInvitados.php?r_codigo="+r_codigo+"&u_codigo="+u_codigo,true);  */
    xmlhttp.open("POST", "http://www.omdbapi.com/?apikey=648317f1&i=" + id_pelicula + "&r=json&plot=full");
    xmlhttp.send();
    /* document.getElementById("tabladatos").disabled=true; */
    //document.getElementById("anterior").disabled = true;
    return false;
...
function cargarInfPelicula(jsonString) {
